...
public class RyanPlayer4 extends Player 
{


	
	public RyanPlayer4(int color) 
	{
		super(color);
	}
	@Override
	public String getName() {
		return "No-Pruning-Optimize-Is-Stable";
	}
	...
	class MiniMax  {
		...
		MiniMax(Board board, int depth, boolean isMaxPlayer, int color, Position moveMade, boolean[][] stableSquares) {
		...
		public int compareTo(MiniMax node) {
			Integer compareEval = this.eval;
			return compareEval.compareTo(node.getEval());
		}
		...
		private void generateMoves() {
			...
			for(Position pos : legalMoves) {
				...
				this.children.add(new MiniMax(childBoard, this.depth+1, !this.isMaxPlayer, this.color, pos, this.stableSquares));
			}
		}
	...
	public Position getNextMove(Board board) {
		...
		MiniMax mini = new MiniMax(board, 0 , true, this.getColor(), null, new boolean[Constants.SIZE][Constants.SIZE]);
