...
public class RyanPlayer3 extends Player 
{
	...
	public String getName() {
		return "Alpha-Beta-Pruning-Optimized-Is-Stable";
	}
	...
	class MiniMax  {
		...
		boolean[][] newStable = new boolean[Constants.SIZE][Constants.SIZE];
		private int alpha;
		private int beta;
		
		MiniMax(Board board, int depth, boolean isMaxPlayer, int color, Position moveMade, int alpha, int beta, boolean[][] stableSquares) {
			this.stableSquares = stableSquares;
			this.newStable = stableSquares;
			this.move = moveMade;
		...
		private void generateMoves() {
			...
			for(int i = 0; i < legalMoves.size(); i++) {
				...
				this.children.add(new MiniMax(childBoard, this.depth+1, !this.isMaxPlayer, this.color, pos, this.alpha, this.beta, this.stableSquares));
		...
		private void updateStable() {
			for(int i = 0; i < Constants.SIZE; i++) {
				for(int j = 0; j < Constants.SIZE; j++) {
					if(!stableSquares[i][j] && boardToEdit.getSquare(new Position(i, j)).getStatus() != 0) {
						isStable(this.boardToEdit, i, j);
					}
				}
			}
			if(newStable != stableSquares) {
				stableSquares = newStable;
				updateStable();
			} else {
				stableSquares = newStable;
			}
		...
		private void isStable(Board board, int x, int y) {
			...
			newStable[x][y] = (nStatic || sStatic) && (neStatic || swStatic) && (nwStatic || seStatic) && (eStatic || wStatic);
		}
	...
	public Position getNextMove(Board board) {
		...
		MiniMax mini = new MiniMax(board, 0 , true, this.getColor(), null, -1000000, 1000000, new boolean[Constants.SIZE][Constants.SIZE]);
