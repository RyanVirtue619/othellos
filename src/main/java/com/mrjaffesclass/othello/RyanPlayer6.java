...
public class RyanPlayer6 extends Player 
{


	
	public RyanPlayer6(int color) 
	{
		super(color);
	}
	...
	public String getName() {
		return "Alpha-Beta-Pruning-NewEvalV2";
	}
	...
	class MiniMax  {
		...
		private int evaluate(Board board) {
			boolean lateGame = cornersTaken() || (board.countSquares(0)  <= 30);
			//Player testPlayer = (this.isMaxPlayer? (new Player(this.color)) : (new Player(this.color*-1)));
			...
			for(int i = 0; i < Constants.SIZE; i++) {
				for(int j = 0; j < Constants.SIZE; j++) {
					...
					if(square.getStatus() != 0) {
						if(!lateGame) {
							if (isCorner(i, j)) {
								newEval += 1000 * square.getStatus();
							} 
						} else {
							if(isStable(boardToEdit,i,j)) {
								newEval += 50 * square.getStatus();
							}
						}
			...
			//newEval += legalMoves.size() * 10;
			newEval = newEval  * color;
			return newEval;
		}
		
		private boolean cornersTaken() {
			return (boardToEdit.getSquare(new Position(0, 0)).getStatus() != 0) && 
				(boardToEdit.getSquare(new Position(Constants.SIZE-1, 0)).getStatus() != 0) && 
				(boardToEdit.getSquare(new Position(0, Constants.SIZE-1)).getStatus() != 0) && 
				(boardToEdit.getSquare(new Position(Constants.SIZE-1, Constants.SIZE-1)).getStatus() != 0) ;
		}
